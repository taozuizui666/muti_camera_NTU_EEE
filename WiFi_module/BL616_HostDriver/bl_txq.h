...
#define BL_TXQ_CLEANUP_INTERVAL (10 * HZ) //10s in jiffies
#define BL_TXQ_MAX_QUEUE_JIFFIES (20 * HZ)
...
#ifdef CONFIG_BL_MUMIMO_TX
#define BL_TXQ_GROUP_ID(txq) ((txq)->mumimo_info & 0x3f)
#define BL_TXQ_POS_ID(txq)   (((txq)->mumimo_info >> 6) & 0x3)
#else
...
/**
 * bl_txq_is_ready_for_push - Check if a TXQ is ready for push
 *
 * @txq: txq pointer
 *
 * if
 ...
 *                   TID order
 *
 * @sta: pointer to bl_sta
 * @txq: pointer to bl_txq updated with the next TXQ at each iteration
 * @tid: int updated with the TXQ tid at each iteration
 * @bl_hw: main driver data
 */
#ifdef CONFIG_MAC80211_TXQ
#define foreach_sta_txq(sta, txq, tid, bl_hw)                         \
    for (tid = 0, txq = bl_txq_sta_get(sta, 0);                       \
         tid < NX_NB_TXQ_PER_STA;                                       \
         tid++, txq = bl_txq_sta_get(sta, tid))
#else /* CONFIG_BL_FULLMAC */
#define foreach_sta_txq(sta, txq, tid, bl_hw)                          \
    for (tid = 0, txq = bl_txq_sta_get(sta, 0, bl_hw);               \
 ...
 */
#define foreach_sta_txq_prio(sta, txq, tid, i, bl_hw)                          \
    for (i = 0, tid = nx_tid_prio[0], txq = bl_txq_sta_get(sta, tid, bl_hw); \
         i < NX_NB_TID_PER_STA;                                                  \
         i++, tid = nx_tid_prio[i], txq = bl_txq_sta_get(sta, tid, bl_hw))

/**
 * foreach_vif_txq - Macro to iterate over all TXQ of a VIF (in AC order)
 *
 * @vif: pointer to bl_vif
 * @txq: pointer to bl_txq updated with the next TXQ at each iteration
 * @ac:  int updated with the TXQ ac at each iteration
 */
#ifdef CONFIG_MAC80211_TXQ
#define foreach_vif_txq(vif, txq, ac)                                   \
    for (ac = BL_HWQ_BK, txq = bl_txq_vif_get(vif, ac);             \
         ac < NX_NB_TXQ_PER_VIF;                                        \
         ac++, txq = bl_txq_vif_get(vif, ac))

#else
#define foreach_vif_txq(vif, txq, ac)                                   \
    for (ac = BL_HWQ_BK, txq = &vif->txqs[0];                         \
         ac < NX_NB_TXQ_PER_VIF;                                        \
         ac++, txq++)
#endif
...
/**
 * bl_txq_vif_get_status - return status bits related to the vif
 *
 * @bl_vif: Pointer to vif structure
 */
static inline u8 bl_txq_vif_get_status(struct bl_vif *bl_vif)
{
    struct bl_txq *txq = bl_txq_vif_get(bl_vif, 0);
...
void bl_txq_unk_vif_deinit(struct bl_vif *vif);
...
void bl_txq_tdls_vif_deinit(struct bl_vif *vif);
