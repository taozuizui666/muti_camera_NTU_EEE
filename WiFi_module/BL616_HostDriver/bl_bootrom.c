...
{
    if ('O' == bootinfo->status[0] && 'K' == bootinfo->status[1]) {
        printk("[RSP] bootinfo versoin: 0x%08X\n",  bootinfo->version);
        printk("[RSP] bootinfo chip_id: 0x%016llX\n",  bootinfo->chip_id);
        ...
        printk("[RSP] unkown status:%c%c\n",
               bootinfo->status[0], bootinfo->status[1]);
        return -1;
    }
...
int bl_bootrom_cmd_bootheader_load(bootrom_host_cmd_t *cmd, 
                                                bootheader_t *header)
{
...
#else /*CONFIG_BL_USB */
    cmd->len = sizeof(bootrom_host_cmd_t) - 
               BL_BOOTROM_HOST_CMD_LEN_HEADER + sizeof(bootheader_t);
#endif
...
{
    if ('O' == bootresponse->status[0] && 'K' == bootresponse->status[1]) {
        printk("[RSP] bootheader_load response OK\n");
    } else if ('F' == bootresponse->status[0] && 'L' == bootresponse->status[1]) {
        printk("[RSP] bootheader_load response failed, staus 0x%04X\n", 
               bootresponse->code);
    } else {
        printk("[RSP] unkown status:%c%c\n",
               bootresponse->status[0], bootresponse->status[1]);
        return -1;
    }
...
int bl_bootrom_cmd_aesiv_load(bootrom_host_cmd_t *cmd, 
                                         const uint8_t *aesiv)
{
    //XXX should caller known the size of cmd->data ?
    cmd->id = BL_BOOTROM_HOST_CMD_AESIV_LOAD;
    //FIXME use struct instead
    cmd->len = sizeof(bootrom_host_cmd_t) - BL_BOOTROM_HOST_CMD_LEN_HEADER + 20;
    //FIXME use struct instead, but NOT magic number(16 IV + 4 CRC32)
    memcpy(cmd->data, aesiv, 20);
...
int bl_bootrom_cmd_pkey1_load(bootrom_host_cmd_t *cmd, pkey_cfg_t *pk)
{
    ...
    cmd->len = sizeof(bootrom_host_cmd_t) - 
               BL_BOOTROM_HOST_CMD_LEN_HEADER + sizeof(pkey_cfg_t);
...
int bl_bootrom_cmd_signature1_load(bootrom_host_cmd_t *cmd,
                                               uint8_t *signature, int len)
{
...
int bl_bootrom_cmd_signature2_load(bootrom_host_cmd_t *cmd, 
                                               uint8_t *signature, int len)
{
...
int bl_bootrom_cmd_sectionheader_load(bootrom_host_cmd_t *cmd,
                                                 const segment_header_t *header)
{
    cmd->id = BL_BOOTROM_HOST_CMD_SECTIONHEADER_LOAD;
    cmd->len = sizeof(bootrom_host_cmd_t) - 
               BL_BOOTROM_HOST_CMD_LEN_HEADER + sizeof(segment_header_t);
...
int bl_bootrom_cmd_sectiondata_load(bootrom_host_cmd_t *cmd, 
...
{
    if ('O' == bootresponse->status[0] && 'K' == bootresponse->status[1]) {
        BL_DBG("[RSP] section_data response OK\n");
    } else if ('F' == bootresponse->status[0] && 'L' == bootresponse->status[1]) {
...
{
    if ('O' == checkimage->status[0] && 'K' == checkimage->status[1]) {
        printk("[RSP] checkimage response OK\n");
        return 0;
    } else if ('F' == checkimage->status[0] && 'L' == checkimage->status[1]) {
        printk("[RSP] checkimage response failed, staus 0x%04X\n", 
               checkimage->code);
    } else {
        printk("[RSP] unkown status:%c%c\n",
               checkimage->status[0], checkimage->status[1]);
    }
    return -1;
}
...
{
    if ('O' == runimage->status[0] && 'K' == runimage->status[1]) {
        printk("[RSP] runimage response OK\n");
    } else if ('F' == runimage->status[0] && 'L' == runimage->status[1]) {
        printk("[RSP] runimage response failed, staus 0x%04X\n", runimage->code);
    } else {
        printk("[RSP] unkown status:%c%c\n",
               runimage->status[0], runimage->status[1]);
        return -1;
    }
...
#endif
{
    ...
    cmd->id = BL_BOOTROM_HOST_CMD_RUN;
#else /*CONFIG_BL_SDIO*/
    cmd->id = BL_BOOTROM_HOST_CMD_RESET;
#endif
    cmd->len = 0;
...
#ifdef CONFIG_BL_SDIO
int bl_bootrom_cmd_efusewrite_get(bootrom_host_cmd_t *cmd, 
                                             uint32_t addr, const uint8_t *data)
{
    bootrom_efuse_write_t *efuse;

    cmd->id = BL_BOOTROM_HOST_CMD_EFUSE_WRITE;
    cmd->len = sizeof(bootrom_efuse_write_t);
    efuse = (bootrom_efuse_write_t*)cmd->data;
    efuse->addr = addr;
    memcpy(efuse->data, data, sizeof(efuse->data));
...
{
    if ('O' == efusewrite->status[0] && 'K' == efusewrite->status[1]) {
        printk("[RSP] efusewrite response OK\n");
        return 0;
    } else if ('F' == efusewrite->status[0] && 'L' == efusewrite->status[1]) {
        printk("[RSP] efusewrite response failed, staus 0x%04X\n", efusewrite->code);
    } else {
        printk("[RSP] unkown status:%c%c\n",
               efusewrite->status[0], efusewrite->status[1]);
    }
...
{
    if ('O' == resetres->status[0] && 'K' == resetres->status[1]) {
        printk("[RSP] resetres response OK\n");
    } else if ('F' == resetres->status[0] && 'L' == resetres->status[1]) {
        printk("[RSP] resetres response failed, staus 0x%04X\n", resetres->code);
    } else {
        printk("[RSP] unkown status:%c%c\n",
               resetres->status[0], resetres->status[1]);
        return -1;
    }
