...
dfs_pattern_detector_init(enum nl80211_dfs_regions region, u8 chain)
{
    ...
    dpd->region = NL80211_DFS_UNSET;
    dpd->enabled = BL_RADAR_DETECT_DISABLE;
    dpd->last_pulse_ts = 0;
    dpd->prev_jiffies = jiffies;
 ...
 * driver interface
 *****************************************************************************/
static u16 bl_radar_get_center_freq(struct bl_hw *bl_hw, u8 chain)
{
    if (chain == BL_RADAR_FCU)
        return bl_hw->phy.sec_chan.center1_freq;

    if (chain == BL_RADAR_RIU) {
        if (!bl_chanctx_valid(bl_hw, bl_hw->cur_chanctx)) {
            WARN(1, "Radar pulse without channel information");
        } else
            return bl_hw->chanctx_table[bl_hw->cur_chanctx].chan_def.center_freq1;
    }

    return 0;
}

static void bl_radar_detected(struct bl_hw *bl_hw)
{
    ...
    /*
      recopy chan_def in local variable because bl_radar_cancel_cac may
    ...
    */
    chan_def = bl_hw->chanctx_table[bl_hw->cur_chanctx].chan_def;

    bl_radar_cancel_cac(&bl_hw->radar);
    cfg80211_radar_event(bl_hw->wiphy, &chan_def, GFP_KERNEL);
}

static void bl_radar_process_pulse(struct work_struct *ws)
{
    struct bl_radar *radar = container_of(ws, struct bl_radar,
                                            detection_work);
    struct bl_hw *bl_hw = container_of(radar, struct bl_hw, radar);
    int chain;
    u32 pulses[BL_RADAR_LAST][BL_RADAR_PULSE_MAX];
    u16 pulses_count[BL_RADAR_LAST];
    ...
    for (chain = BL_RADAR_RIU; chain < BL_RADAR_LAST; chain++) {
        int start, count;

        count = radar->pulses[chain].count;
        start = radar->pulses[chain].index - count;
        if (start < 0)
            start += BL_RADAR_PULSE_MAX;

        pulses_count[chain] = count;
        if (count == 0)
            continue;

        if ((start + count) > BL_RADAR_PULSE_MAX) {
            u16 count1 = (BL_RADAR_PULSE_MAX - start);
    ...
    for (chain = BL_RADAR_RIU; chain < BL_RADAR_LAST; chain++) {
        ...
        freq = bl_radar_get_center_freq(bl_hw, chain);
        ...
        for (i = 0; i < pulses_count[chain] ; i++) {
            ...
            if (dfs_pattern_detector_add_pulse(radar->dpd[chain], chain,
                ...
                if (chain == BL_RADAR_RIU) {
                    ...
                    if (radar->dpd[chain]->enabled == BL_RADAR_DETECT_REPORT) {
                        bl_radar_detected(bl_hw);
                        ...
                        bl_radar_detection_enable(radar,
...
static void bl_radar_cac_work(struct work_struct *ws)
{
    struct delayed_work *dw = container_of(ws, struct delayed_work, work);
    struct bl_radar *radar = container_of(dw, struct bl_radar, cac_work);
    struct bl_hw *bl_hw = container_of(radar, struct bl_hw, radar);
    ...
    ctxt = &bl_hw->chanctx_table[radar->cac_vif->ch_index];
...
#endif
    bl_send_apm_stop_cac_req(bl_hw, radar->cac_vif);
    bl_chanctx_unlink(radar->cac_vif);
...
bool bl_radar_detection_init(struct bl_radar *radar)
{
    ...
    radar->dpd[BL_RADAR_RIU] = dfs_pattern_detector_init(NL80211_DFS_UNSET,
                                                           BL_RADAR_RIU);
    if (radar->dpd[BL_RADAR_RIU] == NULL)
        return false;

    radar->dpd[BL_RADAR_FCU] = dfs_pattern_detector_init(NL80211_DFS_UNSET,
                                                           BL_RADAR_FCU);
    if (radar->dpd[BL_RADAR_FCU] == NULL) {
        bl_radar_detection_deinit(radar);
        return false;
    }

    INIT_WORK(&radar->detection_work, bl_radar_process_pulse);
#ifdef CONFIG_BL_FULLMAC
    INIT_DELAYED_WORK(&radar->cac_work, bl_radar_cac_work);
    radar->cac_vif = NULL;
#endif /* CONFIG_BL_FULLMAC */
    return true;
}
...
{
    if (radar->dpd[BL_RADAR_RIU]) {
        dfs_pattern_detector_exit(radar->dpd[BL_RADAR_RIU]);
        radar->dpd[BL_RADAR_RIU] = NULL;
    }
    if (radar->dpd[BL_RADAR_FCU]) {
        dfs_pattern_detector_exit(radar->dpd[BL_RADAR_FCU]);
        radar->dpd[BL_RADAR_FCU] = NULL;
    }
}
                           ...
                           enum nl80211_dfs_regions region)
{
    ...
    return (dfs_pattern_detector_set_domain(radar->dpd[BL_RADAR_RIU],
                                            region, BL_RADAR_RIU) &&
            dfs_pattern_detector_set_domain(radar->dpd[BL_RADAR_FCU],
                                            region, BL_RADAR_FCU));
}

void bl_radar_detection_enable(struct bl_radar *radar, u8 enable, u8 chain)
{
    if (chain < BL_RADAR_LAST ) {
        spin_lock_bh(&radar->lock);
...
{
    return radar->dpd[chain]->enabled != BL_RADAR_DETECT_DISABLE;
}
 ...
 *****************************************************************************/
static
int bl_radar_dump_pri_detector(char *buf, size_t len,
                                 struct pri_detector *pde)
{
                                     ...
                                     struct bl_radar *radar, u8 chain)
{
    ...
    if (buf == NULL) {
        ...
        for (i = 0; i < dpd->num_radar_types; i++) {
            list_for_each_entry(pde, &dpd->detectors[i], head) {
                size_needed += bl_radar_dump_pri_detector(NULL, 0, pde);
            }
    ...
    for (i = 0; i < dpd->num_radar_types; i++) {
        ...
        len -= res;
        list_for_each_entry(pde, &dpd->detectors[i], head) {
            res = bl_radar_dump_pri_detector(&buf[write], len, pde);
            write += res;
            len -= res;
        }
    }
...
int bl_radar_dump_radar_detected(char *buf, size_t len,
                                   struct bl_radar *radar, u8 chain)
{
    struct bl_radar_detected *detect = &(radar->detected[chain]);
