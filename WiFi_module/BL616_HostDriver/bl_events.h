...
static const char *
bl_ftrace_print_symbols_seq(struct trace_seq *p, unsigned long val,
			 const struct trace_print_flags *symbol_array)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0)
	return trace_print_symbols_seq(p, val, symbol_array);
#else
	int i;
...
#endif
         { -1, NULL }};
    return bl_ftrace_print_symbols_seq(p, hwq_idx, symbols);
}
...
TRACE_EVENT(
    txq_drop_skb,
    TP_PROTO(struct sk_buff *skb, struct bl_txq *txq, unsigned long queued_time),
...
TRACE_EVENT(
    txq_drop,
    TP_PROTO(struct bl_txq *txq, unsigned long nb_drop),
    TP_ARGS(txq, nb_drop),
    TP_STRUCT__entry(
        __field(u16, txq_idx)
        __field(u16, nb_drop)
...
TRACE_EVENT(
    mu_group_update,
    TP_PROTO(struct bl_mu_group *group),
    TP_ARGS(group),
    TP_STRUCT__entry(
        __field(u8, nb_user)
        __field(u8, group_id)
...
TRACE_EVENT(
    mu_group_selection,
    TP_PROTO(struct bl_sta *sta, int group_id),
...
TRACE_EVENT(
    txq_select_mu_group,
    TP_PROTO(struct bl_txq *txq, int group_id, int pos),
...
DECLARE_EVENT_CLASS(
    mesh_path_template,
    TP_PROTO(struct bl_mesh_path *mesh_path),
    TP_ARGS(mesh_path),
    TP_STRUCT__entry(
        __field(u8, idx)
...
TRACE_EVENT(
    ...
    TP_printk("%s: %s radar detection %s",
              ...
              __print_symbolic(__entry->enable,
                               {BL_RADAR_DETECT_DISABLE, "Disable"},
                               {BL_RADAR_DETECT_ENABLE, "Enable (no report)"},
                               {BL_RADAR_DETECT_REPORT, "Enable"}),
              __entry->enable == BL_RADAR_DETECT_DISABLE ? "" :
              __print_symbolic(__entry->region,
                               {NL80211_DFS_UNSET, "UNSET"},
                               {NL80211_DFS_FCC, "FCC"},
                               {NL80211_DFS_ETSI, "ETSI"},
