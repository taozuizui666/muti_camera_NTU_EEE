...
#ifndef _BL_FW_TRACE_H_
#define _BL_FW_TRACE_H_
...
/**
 * struct bl_fw_trace_desc - Trace buffer info as provided by fw in ipc
 *
...
/**
 * struct bl_fw_trace_buf - Info for trace buffer in shared memory
 *
...
/**
 * struct bl_fw_trace_local_buf - Info for a local trace buffer
 *
...
/**
 * struct bl_fw_trace - info to handle several reader of the shared buffer
 *
...
void _bl_fw_trace_dump(struct bl_fw_trace_buf *trace);

int bl_fw_trace_alloc_local(struct bl_fw_trace_local_buf *local,
                              int size);
void bl_fw_trace_free_local(struct bl_fw_trace_local_buf *local);
...
/**
 * bl_fw_trace_empty() - Check if shared buffer is empty
 *
 * @shared_buf: Pointer to shared buffer
 */
static inline bool bl_fw_trace_empty(struct bl_fw_trace_buf *shared_buf)
{
    return (*shared_buf->end >= shared_buf->size);
}

#endif /* _BL_FW_TRACE_H_ */
