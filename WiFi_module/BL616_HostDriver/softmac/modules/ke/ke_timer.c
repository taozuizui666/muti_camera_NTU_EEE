                      ...
                      uint32_t const delay)
{
    bool hw_prog = false;
    uint64_t abs_time;
    struct ke_timer *first = NULL;
    struct ke_timer *timer = NULL;
    ...
    timer = (struct ke_timer*)ke_queue_extract(&ke_env.queue_timer, cmp_timer_id, 
    ...
    abs_time = softmac_time() + usecs_to_jiffies(delay);
    timer->time = abs_time;
    timer->delay = delay;

    dbg("%s, %llu %llu %u\r\n", __func__, abs_time, softmac_time(), delay);

    // insert in sorted timer list
    co_list_insert(&ke_env.queue_timer, (struct co_list_hdr*) timer,
                   cmp_abs_time);
...
void ke_timer_clear(ke_msg_id_t const timer_id, ke_task_id_t const task_id)
{
    ...
    if (ke_env.queue_timer.first != NULL)
    {
        if ((timer->id == timer_id) && (timer->task == task_id))
        {
            ...
            // Check that the timer did not expire before HW prog
            ASSERT_ERR(!first || !softmac_time_past(first->time));
        }
        else
        {
...
void ke_timer_schedule(int dummy)
{
    ...
    for(;;)
    {
        ...
        if (!softmac_time_past(timer->time - 50))
        {
            ...
            if (!softmac_time_past(timer->time))
                break;
        }
...
/**
 ****************************************************************************************
 * @brief Remove all the programmed timers.
 ****************************************************************************************
 */
void ke_timer_reset(void)
{
    ...
    // Clear the timer event in case it would have been scheduled just before
    ke_evt_clear(KE_EVT_KE_TIMER_BIT);

    for(;;)
    {
        // Pop the timer from the list until no more is programmed
        timer = (struct ke_timer*) ke_queue_pop(&ke_env.queue_timer);
        if (timer == NULL)
            break;
