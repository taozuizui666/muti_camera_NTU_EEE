...
#if KE_PROFILING
/// Change return addr from caller.
#include "riscv_encoding.h"

#define KE_MSG_ALLOC_BASIC(id, dest, src, var_len) ({ uint32_t * msg; \
    msg = ke_msg_alloc(id, dest, src, var_len); \
    *(msg - 3 - 2) = read_gr(ra); \
    msg; })

#define KE_MSG_ALLOC(id, dest, src, param_str) ({ uint32_t * msg; \
    msg =  (uint32_t *)ke_msg_alloc(id, dest, src, sizeof(struct param_str)); \
    *(msg - 3 - 2) = read_gr(ra); \
    (struct param_str *) msg; })

#define KE_MSG_ALLOC_VAR(id, dest, src, param_str, var_len) ({ uint32_t * msg; \
    msg = ke_msg_alloc(id, dest, src, sizeof(struct param_str) + var_len); \
    *(msg - 3 - 2) = read_gr(ra); \
    (struct param_str *) msg; })
 ...
 ****************************************************************************************
 */
#define KE_MSG_ALLOC_BASIC(id, dest, src, var_len) \
    ke_msg_alloc(id, dest, src, var_len)
