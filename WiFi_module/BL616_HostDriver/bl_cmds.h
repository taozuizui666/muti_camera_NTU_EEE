...
#ifdef CONFIG_BL_SDM
#define BL_80211_CMD_TIMEOUT_MS    (15 * 1000)
#else
#define BL_80211_CMD_TIMEOUT_MS   (15 * 1000) 
#endif
...
#define BL_CMD_FLAG_DNLDING       BIT(6)
#define BL_CMD_FLAG_DNLD_COMPLETE BIT(7)
...
#define BL_CMD_MAX_QUEUED         16

#define bl_cmd_e2amsg ipc_e2a_msg
#define bl_cmd_a2emsg lmac_msg
#define BL_CMD_A2EMSG_LEN(m) (sizeof(struct lmac_msg) + m->param_len)
#define BL_CMD_E2AMSG_LEN_MAX (IPC_E2A_MSG_PARAM_SIZE * 4)

struct bl_hw;
struct bl_cmd;
typedef int (*msg_cb_fct)(struct bl_hw *bl_hw, struct bl_cmd *cmd,
                          struct bl_cmd_e2amsg *msg);
...
struct bl_cmd_mgr {
    ...
    int  (*msgind)(struct bl_cmd_mgr *, struct bl_cmd_e2amsg *, msg_cb_fct);
