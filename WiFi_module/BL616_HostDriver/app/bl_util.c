...
static int bl_util_scan(int argc, char ** argv, u32 cmd_id)
{
    int ret = 0, i = 0;
    struct iwreq wrq;
    //argc=4, interface wlan0
    //argv[0]=./blutl
    //argv[1]=wlan0
    //argv[2]=scan
    //argv[3]=0 or 1
...
static int bl_util_scan_chan(int argc, char ** argv, u32 cmd_id)
{
    int ret = 0, i = 0;
    struct iwreq wrq;
    //argc=7, interface wlan0
    //argv[0]=./blutl
    //argv[1]=wlan0
    //argv[2]=scan_chan
    //argv[3]=1
...
static int bl_util_scan_ie(int argc, char ** argv, u32 cmd_id)
{
    int ret = 0, i = 0;
    struct iwreq wrq;
    //argc=7, interface wlan0
...
static int bl_util_get_country_code(int argc, char ** argv, u32 cmd_id)
{
    int ret = BL_UTIL_SUCCESS, i = 0;
    struct iwreq wrq;
    //argv[0]=./blutl
    //argv[1]=wlan0
    //argv[2]=country_code
    int8_t country_code[3];
	
    ret = bl_util_send_cmd(argc, argv, &wrq, TYPE_BYTE, cmd_id);
    if (ret) {
        printf("Error: blutl version fail %d\n", ret);
    } else {
        printf("%s:%d \n", __func__, *(u8 *)(wrq.u.data.pointer + sizeof(cmd_id)));
    }
...
static int bl_util_get_mac_addr(int argc, char ** argv, u32 cmd_id)
{
    int ret = BL_UTIL_SUCCESS, i = 0;
    struct iwreq wrq;
	u8  mac[6];
    ...
    if (ret) {
        ...
        memcpy(mac, (u8 *)(wrq.u.data.pointer + sizeof(cmd_id)), 6);
...
static int bl_util_get_intf_status(int argc, char ** argv, u32 cmd_id)
{
    int ret = BL_UTIL_SUCCESS, i = 0;
    struct iwreq wrq;
    //argv[0]=./blutl
...
static int bl_util_get_rssi(int argc, char ** argv, u32 cmd_id)
{
    int ret = BL_UTIL_SUCCESS, i = 0;
    struct iwreq wrq;
    //argv[0]=./blutl
    ...
    if (ret) {
        ...
        printf("%s: rssi %d \n", __func__, *(s8 *)(wrq.u.data.pointer + sizeof(cmd_id)));
    }
...
static int bl_util_read_efuse(int argc, char ** argv, u32 cmd_id)
{
    int ret = BL_UTIL_SUCCESS, i = 0;
    struct iwreq wrq;
    ...
    if (ret) {
        ...
        memcpy((u8 *)&data, (u8 *)(wrq.u.data.pointer + sizeof(cmd_id)), sizeof(struct efuse_data));
       // printf("%s:%d \n", __func__, *(u8 *)(wrq.u.data.pointer + sizeof(cmd_id)));
       printf("xtal 0x%x \n", data.xtal_value);	   
	   printf("mac %x : %x : %x : %x : %x: %x \n", (u8)data.mac[0], (u8)data.mac[1], (u8)data.mac[2], (u8)data.mac[3], (u8)data.mac[4], (u8)data.mac[5]);	   
	   printf("pwr offset : 0x%x \n", data.pwr_offset[0], data.pwr_offset[6], data.pwr_offset[12]);	
    }
...
struct util_cmd_node bl_util_cmd_table[] = {
    ...
    {"scan",        BL_UTIL_CMD_SCAN,               bl_util_scan},
    {"scan_chan",   BL_UTIL_CMD_SCAN_CHAN,          bl_util_scan_chan},
    {"scan_ie",     BL_UTIL_CMD_SCAN_IE,            bl_util_scan_ie},
    {"country_code",      BL_UTIL_CMD_GET_COUNTRY_CODE,          bl_util_get_country_code},
    {"mac_addr",          BL_UTIL_CMD_GET_MAC_ADDR,              bl_util_get_mac_addr},
    {"status",            BL_UTIL_CMD_GET_STATUS,                bl_util_get_intf_status},
    {"rssi",              BL_UTIL_CMD_GET_RSSI,                  bl_util_get_rssi},
    {"read_efuse",        BL_UTIL_CMD_READ_EFUSE,                bl_util_read_efuse},
};
...
void bl_util_event_loop(void)
{
    ...
    while (1) {
        ...
        switch (bl_event->event_id) {
            case BL_EVENT_ID_PROBE_RESPONSE:
                {
                    bl_util_parsing_mgmt_frame(bl_event->payload, bl_event->payload_len);
                }
                break;

            case BL_EVENT_ID_SCAN_DONE:
                {
                    printf("BL_EVENT_ID_SCAN_DONE Finished\n");
                }
                break;
            case BL_EVENT_ID_RESET:
            {
                ...
                break;
            case BL_EVENT_ID_OUT_MEM:
            {
                printf("Driver out of memory , need flush the catch\n");
            }
                break;
