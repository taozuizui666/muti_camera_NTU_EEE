                                        ...
                                        size_t count, loff_t *ppos)
{
    ...
    if (!priv->debugfs.trace_prst) {
        ...
        bl_dbgfs_trigger_fw_dump(priv, msg);
        return 0;
    }
...
int bl_um_helper(struct bl_debugfs *bl_debugfs, const char *cmd)
{
    ...
    if ((ret = call_usermodehelper(argv[0], argv, envp,
        ...
        dev_err(bl_hw->dev, "Failed to call %s (%s returned %d)\n",
                argv[0], cmd, ret);
    argv_free(argv);
#endif

    int result = 0;
    char cmd_path[] = "/bin/bash";
    char* cmd_argv[] = {cmd_path,"-c", "/bin/dmesg | grep fwlog >> /tmp/fw_log", NULL};
    char* cmd_envp[] = {NULL};
    result = call_usermodehelper(cmd_path, cmd_argv, cmd_envp, 2);
    BL_DBG("result=%d\n", result);

    return result;
}

void bl_um_helper_work(struct work_struct *ws)
{
    struct bl_debugfs *bl_debugfs = container_of(ws, struct bl_debugfs,
                                                     helper_work);
    struct bl_hw *bl_hw = container_of(bl_debugfs, struct bl_hw,
                                           debugfs);
    bl_um_helper(bl_debugfs, NULL);
    if (!bl_debugfs->unregistering)
        bl_umh_done(bl_hw);
    bl_debugfs->helper_scheduled = false;
}
...
int bl_trigger_um_helper(struct bl_debugfs *bl_debugfs)
{
    ...
    if (bl_debugfs->helper_scheduled == true) {
        //dev_err(bl_hw->dev, "%s: Already scheduled\n", __func__);
        return -EBUSY;
    }
...
#endif
        dev_err(bl_hw->dev, "%s: unregistering\n", __func__);
        return -ENOENT;
    }
...
{
    flush_work(&bl_hw->debugfs.helper_work);
}
