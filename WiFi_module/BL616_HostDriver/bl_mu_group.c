...
/**
 * bl_mu_group_sta_init - Initialize group information for a STA
 *
                                 ...
                                 const struct ieee80211_vht_cap *vht_cap)
{
    ...
    if (!vht_cap ||
        !(vht_cap->vht_cap_info & IEEE80211_VHT_CAP_MU_BEAMFORMEE_CAPABLE)) {
            sta->group_info.map = BL_SU_GROUP;
    }
}

/**
 * bl_mu_group_sta_del - Remove a sta from all MU group
 *
...
void bl_mu_group_sta_del(struct bl_hw *bl_hw, struct bl_sta *sta)
{
    ...
    group_sta_for_each(sta, group_id, map) {
        struct bl_mu_group *group = bl_mu_group_from_id(mu, group_id);
...
/**
 * bl_mu_group_sta_get_map - Get the list of group a STA belongs to
 *
...
/**
 * bl_mu_group_sta_get_pos - Get sta position in a group
 *
 * @bl_hw: main driver data
 * @sta: pointer to the sta
 * @group_id: Group id
                                      ...
                                      int group_id)
{
    ...
    group = bl_mu_group_from_id(&bl_hw->mu, group_id);
    if (!group)
        return -1;
...
/**
 * bl_mu_group_move_head - Move (or add) one element at the top of a list
 *
 * @list: list pointer
 ...
 */
static inline
void bl_mu_group_move_head(struct list_head *list, 
...
/**
 * bl_mu_group_remove_users - Remove all the users of a group
 *
 ...
 */
static inline
void bl_mu_group_remove_users(struct bl_mu_info *mu,
                                          struct bl_mu_group *group)
{
    struct bl_sta *sta;
    ...
    for (i = 0; i < CONFIG_USER_MAX; i++) {
        if (group->users[i]) {
            ...
            bl_mu_group_move_head(&mu->update_sta,
...
/**
 * bl_mu_group_add_users - Add users to a group
 *
 * @mu: pointer on MU info
 ...
 */
static inline
void bl_mu_group_add_users(struct bl_mu_info *mu,
                                     struct bl_mu_group *group,
                                     int nb_user, struct bl_sta **users)
{
...
/**
 * bl_mu_group_create_one - create on group with a specific group of user
 *
 * @mu: pointer on MU info
 ...
 */
static
int bl_mu_group_create_one(struct bl_mu_info *mu, int nb_user,
                                     struct bl_sta **users, int *nb_group_left)
{
    int i, group_id;
    struct bl_mu_group *group;
    u64 group_match;
    ...
    if (group_match) {
        ...
        group_id = BL_GET_FIRST_GROUP_ID(group_match);
        group = bl_mu_group_from_id(mu, group_id);
        bl_mu_group_move_head(&mu->active_groups, &group->list);
        return 0;
    }
    ...
    if (group_avail) {
        ...
        struct bl_sta *users2[CONFIG_USER_MAX];
        int nb_user2;
        ...
        group_for_each(group_id, group_avail) {
            ...
            if ((group->user_cnt + nb_user2) <= CONFIG_USER_MAX) {
                bl_mu_group_add_users(mu, group, nb_user2, users2);
                bl_mu_group_move_head(&mu->active_groups, &group->list);
                return 0;
            }
        }
    ...
    /* create a new group */
    group = list_last_entry(&mu->active_groups, struct bl_mu_group, list);
    bl_mu_group_remove_users(mu, group);
    bl_mu_group_add_users(mu, group, nb_user, users);
    bl_mu_group_move_head(&mu->active_groups, &group->list);
    (*nb_group_left)--;
...
/**
 * bl_mu_group_create - Create new groups containing one specific sta
 *
 * @mu: pointer on MU info
 ...
 */
static
void bl_mu_group_create(struct bl_mu_info *mu, struct bl_sta *sta,
                                int *nb_group_left)
{
    struct bl_sta *user_sta = sta;
    ...
    while (*nb_group_left) {
        ...
        if (nb_user > 1) {
            if (bl_mu_group_create_one(mu, nb_user, users, nb_group_left))
                (*nb_group_left)--;
...
/**
 * bl_mu_group_work - process function of the "group_work"
 *
 ...
 */
void bl_mu_group_work(struct work_struct *ws)
{
    struct delayed_work *dw = container_of(ws, struct delayed_work, work);
    struct bl_mu_info *mu = container_of(dw, struct bl_mu_info, group_work);
    struct bl_hw *bl_hw = container_of(mu, struct bl_hw, mu);
    struct bl_sta *sta, *next;
    int nb_group_left = NX_MU_GROUP_MAX;

    if (WARN(!bl_hw->mod_params->mutx,
    ...
    list_for_each_entry_safe(sta, next, &mu->active_sta, group_info.active) {
        if (nb_group_left)
            bl_mu_group_create(mu, sta, &nb_group_left);
    ...
    if (! list_empty(&mu->update_sta)) {
        list_for_each_entry_safe(sta, next, &mu->update_sta, group_info.update) {
            bl_send_mu_group_update_req(bl_hw, sta);
            list_del(&sta->group_info.update);
        }
    }

    mu->next_group_select = jiffies;
    bl_mu_group_sta_select(bl_hw);
    up(&mu->lock);
...
/**
 * bl_mu_group_init - Initialize MU groups
 *
 * @bl_hw: main driver data
 *
...
void bl_mu_group_init(struct bl_hw *bl_hw)
{
    ...
    mu->group_cnt = 0;
    mu->next_group_select = jiffies;
    INIT_DELAYED_WORK(&mu->group_work, bl_mu_group_work);
    sema_init(&mu->lock, 1);
}

/**
 * bl_mu_set_active_sta - mark a STA as active
 *
 * @bl_hw: main driver data
 * @sta: pointer to the sta
                                 ...
                                 int traffic)
{
    ...
    if (!sta || (sta->group_info.map & BL_SU_GROUP))
        return;
    ...
    if ((sta->group_info.last_update != mu->update_count) ||
        ...
        bl_mu_group_move_head(&mu->active_sta, &sta->group_info.active);

        if (!delayed_work_pending(&mu->group_work) &&
            !list_is_singular(&mu->active_sta)) {
            schedule_delayed_work(&mu->group_work,
                                  msecs_to_jiffies(BL_MU_GROUP_INTERVAL));
        }
    }
}

/**
 * bl_mu_set_active_group - mark a MU group as active
 *
 * @bl_hw: main driver data
 * @group_id: Group id
 *
...
/**
 * bl_mu_group_sta_select - Select the best group for MU stas
 *
 * @bl_hw: main driver data
 *
 ...
 * for each group for the previous @BL_MU_GROUP_SELECT_INTERVAL interval:
...
void bl_mu_group_sta_select(struct bl_hw *bl_hw)
{
    ...
    list_for_each_entry(vif, &bl_hw->vifs, list) {
        ...
        list_for_each_entry(sta, head, list) {
            ...
            if (sta->group_info.cnt == 0 ||
                sta_traffic < BL_MU_GROUP_MIN_TRAFFIC)
                continue;
    ...
    mu->next_group_select = jiffies +
        msecs_to_jiffies(BL_MU_GROUP_SELECT_INTERVAL);
