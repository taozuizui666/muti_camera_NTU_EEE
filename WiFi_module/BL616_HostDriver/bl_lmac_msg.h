...
enum rxu_msg_tag
{
    ...
    RXU_MGT_IND = LMAC_FIRST_MSG(TASK_RXU),
...
enum mm_msg_tag
{
    ...
    MM_SET_DTIM_CFM,
    /// BEACON INTERVAL Configuration Request.      20
    ...
    MM_SET_MODE_CFM,
    /// Request setting the VIF active state (i.e associated or AP started)     30
    ...
    /// KEY DEL Confirm.
    MM_KEY_DEL_CFM,
    /// Block Ack agreement info addition                             40
    MM_BA_ADD_REQ,
    ...
    /// Set Power Save mode
    MM_SET_PS_MODE_REQ,
    /// Set Power Save mode confirmation                              50
    MM_SET_PS_MODE_CFM,
    ...
    /// Confirmation of the channel context update                    60
    ...
    /// Request to remain on channel or cancel remain on channel               70
    ...
    /// Message containing Beamformer information             80
    MM_BFMER_ENABLE_REQ,
    ...
    MM_CSA_TRAFFIC_IND,
    /// Request to update the group information of a station          90
    ...
    MM_SET_UORA_REQ,
    /// UORA PARAMETERS Configuration Confirmation.                100
    MM_SET_UORA_CFM,
...
struct mm_version_cfm
{
    ...
    /// mac addr
    u8_l mac[6];
    /// sub Version of the LMAC FW
    u32_l sub_version;
    /// efuse mac slot
    int8_t mac_slot;
    char version_signed[20];
    u32_l chip_ver;    
    uint32_t sync_features;
};
...
struct mm_sta_del_req
{
    ...
    u8_l sta_idx;
    //vif_idx only valid when as sta, called by sm, no apm or me
    uint8_t vif_idx;
};
...
struct mm_set_phy_misc_req
{
    uint32_t misc_type;
    int32_t misc_value[50];
    //misc_type = 1, set cca, format of misc_value:
    //uint8_t cca_rise;
    //uint8_t cca_fall;
};

struct mm_set_phy_misc_cfm
{
    uint32_t misc_type;
    uint32_t print_type;  //0, print in hex.  1 print in string
    uint32_t len;         //len of resp array
    int32_t resp[50];
};
...
#define WL_WLAN_CHANNELS_NUM           (14)
#define WL_BZ_CH_PWRCOMP_NUM           (5)
struct wl_pwrcal_t
{
    int8_t      channel_pwrcomp_wlan[WL_WLAN_CHANNELS_NUM];
    int8_t      channel_pwrcomp_bz[WL_BZ_CH_PWRCOMP_NUM];
    int8_t      Temperature_MP; // temperature of sensor while power cal at production line
};

struct wl_pwrtarget_t
{
    int8_t      pwr_11b[4];
    int8_t      pwr_11g[8];
...
struct mm_cal_cfg_req
{
    uint8_t xtal_mode_valid;
    uint8_t xtal_cap_valid;
    uint8_t pwr_mode_valid;
    uint8_t pwr_11b_valid;
    uint8_t pwr_11g_valid;
    uint8_t pwr_11n_ht20_valid;
    uint8_t pwr_11n_ht40_valid;
    uint8_t pwr_11ac_vht20_valid;
    uint8_t pwr_11ac_vht40_valid;
    uint8_t pwr_11ac_vht80_valid;
    uint8_t pwr_11ax_he20_valid;
    uint8_t pwr_11ax_he40_valid;
    uint8_t pwr_11ax_he80_valid;
    uint8_t pwr_11ax_he160_valid;
    uint8_t tcal_valid;
    uint8_t mac_valid;
    uint8_t pwr_oft_wifi_valid;
    uint8_t capcode_valid;

    uint8_t pwr_ble_valid;
    uint8_t pwr_bt_valid;
    uint8_t pwr_zb_valid;
    uint8_t pwr_oft_bz_valid;
    ...
    struct wl_pwrcal_t pwrcal;
    struct wl_pwrtarget_t pwrtarget;
    struct wl_tcal_t tcal;
...
#define SCAN_CHANNEL_MAX (MAC_DOMAINCHANNEL_24G_MAX)

/// Maximum number of PHY bands supported
#define SCAN_BAND_MAX 1
#endif

/// Maximum length of the ProbeReq IEs (SoftMAC mode)
#define SCAN_MAX_IE_LEN 600
...
struct scan_start_req
{
    ...
    /// BSSID to be scanned
    struct mac_addr bssid;
    /// Pointer (in host memory) to the ProbeReq with the additional IEs 
    u8_l add_ies[SCAN_MAX_IE_LEN];
...
struct rc_rate_stats
{
    ...
    u16_l probability;
    /// 100ms probability of success
    uint16_t cur_probability;
    /// last time 100ms probability
    uint16_t last_pro;
    uint32_t last_time;
    uint8_t is_11b_11g_rate;
...
struct coex_param {
    //not pct like param
    uint8_t coex_cfg_split_num;

    //pct like param, one byte for each, group as below.
    uint8_t coex_cfg_wifi_pct_bt_inq_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_inq_wifi_con;

    uint8_t coex_cfg_wifi_pct_bt_sco_trx_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_sco_trx_wifi_con;

    uint8_t coex_cfg_wifi_pct_bt_acl_trx_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_acl_trx_wifi_con;

    uint8_t coex_cfg_wifi_pct_bt_acl_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_acl_wifi_con;

    uint8_t coex_cfg_wifi_pct_bt_pi_scan_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_pi_scan_wifi_con;

    uint8_t coex_cfg_wifi_pct_bt_other_wifi_con_trx;
    uint8_t coex_cfg_wifi_pct_bt_other_wifi_con;
} __packed;

struct dbg_coex_rw_param_req {
    uint8_t rw_cmd;
    struct coex_param cp;
} __packed;

struct dbg_coex_rw_param_cfm {
    uint8_t status;
    uint8_t cur_wifi_pct;
    struct coex_param cp;
} __packed;
...
struct dbg_ke_stat_req
{
    bool clear_on_read;
    uint32_t period_rpt;
    uint32_t period_print;
};

struct ke_statistic
{
    uint32_t v_ts;   //higher 10 bits is ver, other 22 bits is timestamp in ms unit
    
    //accumlating counting
    uint32_t rxl_err_cnt;
    uint32_t rxu_err_cnt;
    uint32_t rxl_rx_cnt;
    uint32_t rxu_rx_cnt;
    uint32_t intf_upld_cnt;

    uint32_t intf_err_cnt;
    uint32_t intf_tight_cnt;

    uint32_t intf_dnld_cnt;
    uint32_t txu_tx_cnt;
    uint32_t txu_tx_succ_cnt;
    uint32_t txu_tx_fail_cnt;
    uint32_t txu_tx_retry_cnt;
    uint32_t txl_int_tx_cnt;
    uint32_t txu_err_cnt;
    uint32_t txl_err_cnt;
    uint32_t txu_postpone_accu_cnt;

    uint32_t rec_cnt;
    uint32_t rer_cnt;
    
    //on time counting.
    uint32_t txu_postpone_cnt;

    uint32_t cfm_cnt[5];
    uint32_t txing_cnt[5];
    uint32_t frm_free_cnt;

    uint32_t buf_alloc_cnt;
    uint32_t buf_free_cnt;

    uint32_t bam_tx_delay_cnt[4];
    uint32_t sta_pp_cnt[6][5];
};

struct dbg_ke_stat_cfm
{
    struct ke_statistic ke_statis;
};
...
struct dbg_btble_uart_baud_req {
    int baud;
    int flow;
    int rts;
    int cts;
...
struct dbg_btble_uart_baud_cfm {
    int status;
};
